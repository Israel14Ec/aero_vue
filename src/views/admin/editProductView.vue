<script setup>
    ...
    // Consultar FireStore
    const route = useRoute() //usamos router para obtener el di del url
    const router = useRouter()
    const db = useFirestore()
    const docRef = doc(db, 'products', route.params.id)
    const product = useDocument(docRef) //obtengo los datos
    ...
    //Con el watch puede acceder mas facil acceder a los datos y mostrar
    watch(product, (product) => {

        if(!product){
            router.push({name: 'products'})
        }
        Object.assign((formData), product) //Estoy asignando los datos al reactive
    })

    //Este es el boton del formulario
